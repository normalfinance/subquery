...
//     const switchName = operation?.["switch"]?.name;
...
//     const val =
//       operation?.["value"]?.attributes?.data?._value?._attributes?.val;
...
//                   ).toString("hex")
//                 );
//                 logger.debug(`🔍 Found TokenA: ${JSON.stringify(tokenA)}`);
//               } else if (
//                 symbolName === "TokenB" &&
//                 itemValue?.switch?.().name === "scvAddress"
//               ) {
...
//                 logger.debug(
//                   `🔍 Found Reserves: ${reserveA.toString()}, ${reserveB.toString()}`
//                 );
//               } else if (
...
//               ) {
//                 fee = BigInt(itemValue.u32().toString());
//                 logger.debug(`🔍 Found FeeFraction: ${fee.toString()}`);
...
//       const rawKey =
//         operation?.["value"]?._attributes?.data?._value?._attributes?.key;
//       const key = rawKey?._value === 1 ? "reserveA" : "reserveB";
//       const rawReserve =
//         operation?.["value"]?._attributes?.data?._value?._attributes?.val;
...
//   logger.debug(
//     `🟢 Reserves: ReserveA=${reserveA?.toString() || "not found"}, ReserveB=${
//       reserveB?.toString() || "not found"
//     }, Fee=${fee?.toString() || "not found"}`
//   );
...
//     logger.debug(`🔍 Generated ledger key for ${contractId}: ${xdrBase64}`);
